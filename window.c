...
/* Parse a color string into both XftColor and a plain pixel value.
 * Falls back to black on failure. */
static void load_color(Viewer *v, SpdfColor *c, const char *spec)
{
    ...
    /* Derive plain pixel from the XRenderColor (already resolved) */
    XColor xc;
...
/* Try each comma-separated font name in order; return first that loads. */
static XftFont *load_font(Viewer *v, const char *spec)
{
    /* Make a writable copy to tokenize */
...
static void draw_bar(Viewer *v)
{
    ...
    /* Build status string */
    char left[512], right[128];
    const char *fit_str = "";
    switch (v->fit) {
        case FIT_WIDTH:  fit_str = " [W]"; break;
        case FIT_HEIGHT: fit_str = " [H]"; break;
        case FIT_PAGE:   fit_str = " [F]"; break;
        default: break;
    }
...
void win_draw(Viewer *v)
{
    ...
    int page_y = topbar ? v->bar_h : 0;
    int page_h = v->win_h - v->bar_h;
    ...
    /* Status bar (always on top of everything) */
    draw_bar(v);
}
...
int win_init(Viewer *v)
{
    ...
    /* Load colors before creating window so bgcolor is ready */
    load_colors(v);

    /* Load font to know bar height */
...
void win_toggle_fullscreen(Viewer *v)
{
    if (!v->fullscreen) {
        ...
        /* XGetWindowAttributes gives coords relative to parent;
         * translate to root to get real screen position */
        Window child;
        ...
        if (mwm != None) {
            long hints[5] = { 2, 0, 0, 0, 0 }; /* MWM_HINTS_DECORATIONS = 2, decorations = 0 */
            XChangeProperty(v->dpy, v->win, mwm, mwm,
                32, PropModeReplace, (unsigned char*)hints, 5);
        }

        /* Also send _NET_WM_STATE_FULLSCREEN for WMs that support it */
        XEvent e = {0};
        ...
        /* Move and resize to cover full screen */
        ...
        Atom mwm = XInternAtom(v->dpy, "_MOTIF_WM_HINTS", False);
        if (mwm != None) {
            long hints[5] = { 2, 0, 1, 0, 0 }; /* decorations = 1 */
            XChangeProperty(v->dpy, v->win, mwm, mwm,
                32, PropModeReplace, (unsigned char*)hints, 5);
        }
